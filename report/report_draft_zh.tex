...
我们没有从头训练模型，而是使用预训练的 Grounding DINO 模型进行 zero-shot 推理。项目中主要使用 HuggingFace 模型接口，包括 Grounding DINO tiny 和 Grounding DINO base。考虑到 100 张图片实验的运行时间，本报告中的主要定量实验使用 \texttt{grounding-dino-tiny}。在小规模 smoke test 中也尝试了 \texttt{grounding-dino-base}，但在 20 张图上的结果与 tiny 接近，因此最终报告重点分析更完整的 100 张 tiny 实验。
...
我们构建了两个子集：20 张图片 smoke test 子集用于快速验证流程，100 张图片正式实验子集用于主要定量分析。100 张子集包含 100 张图片、20 个类别和 660 个标注框。数据准备脚本会保留 COCO annotation 格式，方便后续使用 COCOeval 进行标准目标检测评估。
...
配置项 & 设置 \\
\midrule
模型 & \texttt{IDEA-Research/grounding-dino-tiny} \\
图像数量 & 100 \\
类别数量 & 20 \\
box threshold & 0.25 \\
text threshold & 0.25 \\
...
最初使用默认 threshold：box threshold = 0.40，text threshold = 0.30。在 20 张 smoke test 上，AP 仅为 0.0586，AR100 仅为 0.0702，说明模型漏检严重。将 threshold 降至 0.25 / 0.25 后，AP 提升到 0.2866，AP50 提升到 0.3801，说明较低阈值能显著增加召回。但继续降低到 0.20 / 0.20 后，候选框数量进一步增加，误检增多，AP 下降到 0.2453。因此后续实验采用 box threshold = 0.25，text threshold = 0.25。

\subsubsection{Multi-class Prompt 与 Per-class Prompt 对比}

100 张 COCO 子集上的主要结果如表 \ref{tab:main-results} 所示。

\begin{table}[H]
\centering
\caption{100 张 COCO 子集实验结果}
...
per-class prompt + NMS & \textbf{0.2355} & \textbf{0.2832} & \textbf{0.2669} & \textbf{0.5229} & 2711 & \textbf{0} \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{figures/threshold_comparison_100.png}
\caption{100 张 COCO 子集实验对比}
\end{figure}

\subsubsection{每类 AP 分析}

在 multi-class prompt 方法中，表现较好的类别包括 laptop、person、motorcycle、couch、bus 等。per-class prompt + NMS 后，部分类别明显改善，如表 \ref{tab:class-improvement} 所示。

\begin{table}[H]
\centering
\caption{per-class prompt + NMS 对部分类别 AP 的提升}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{figures/per_class_ap_100_perclass_nms.png}
\caption{改进方法每类 AP}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\linewidth]{figures/baseline_false_negative_case.jpg}
\caption{Baseline false negative case}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\linewidth]{figures/perclass_false_positive_case.jpg}
